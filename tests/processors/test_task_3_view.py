...
_CONTENT = {
    "text": "Фрагмент текста для анализа.",
    "statements": ["Утв1", "Утв2", "Утв3", "Утв4", "Утв5"],
}
_EXPL = "Разбор утверждений."
...
class TestTask3View:
    async def test_create_task_builds_view(self, processor_factory, user_factory, category_factory, exercise_factory):
        cat = await category_factory(name="T3", handler_type=HandlerType.TASK_3_EXAM)
        await exercise_factory(category_id=cat.id, content=_CONTENT, answer="135", explanation=_EXPL)
        user = await user_factory()
        processor = processor_factory.get_processor(HandlerType.TASK_3_EXAM)
        ...
        assert out.startswith("### Задание 3\n\n")
        assert "\n\n---\n\n" in out
        assert "<details open><summary>Текст</summary>" in out
        assert "> Фрагмент текста для анализа." in out
        assert "1. Утв1" in out
        assert "5. Утв5" in out

    async def test_process_answer_correct_collapses_explanation(
        self, processor_factory, user_factory, category_factory, exercise_factory,
    ):
        cat = await category_factory(name="T3", handler_type=HandlerType.TASK_3_EXAM)
        ex = await exercise_factory(category_id=cat.id, content=_CONTENT, answer="135", explanation=_EXPL)
        user = await user_factory()
        processor = processor_factory.get_processor(HandlerType.TASK_3_EXAM)
        result = await processor.process_answer(_user_ex(user, cat, [ex]), "531")
        ...
        assert "Разбор утверждений." in out
        assert "<details><summary>Варианты ответов</summary>" in out

    async def test_process_answer_wrong_opens_explanation(
        self, processor_factory, user_factory, category_factory, exercise_factory,
    ):
        cat = await category_factory(name="T3", handler_type=HandlerType.TASK_3_EXAM)
        ex = await exercise_factory(category_id=cat.id, content=_CONTENT, answer="135", explanation=_EXPL)
        user = await user_factory()
        processor = processor_factory.get_processor(HandlerType.TASK_3_EXAM)
        result = await processor.process_answer(_user_ex(user, cat, [ex]), "12")
        assert result.is_correct is False
        out = RichRenderer().render_result(result.result_view)
        # task 3 (как в config.py): «Ваш ответ» НЕ зачёркивается
